...
# MAGIC %md
# MAGIC # Exercise 1 - Solution SQL

# COMMAND ----------

# DBTITLE 1,Exercise 1
# MAGIC %sql
# MAGIC DELETE FROM fake_data WHERE total_spend < 5000;
# MAGIC
...
# MAGIC %sql
# MAGIC RESTORE TABLE fake_data TO VERSION AS OF 4

# COMMAND ----------

# MAGIC %md
# MAGIC # Exercise 1 - Solution Python

# COMMAND ----------

#Print the initial count
display(spark.read.table("fake_data").count())

# COMMAND ----------

#Delete the rows where total_spend is < 5000 by filtering them out (where condition in SQL statement)
spark.read.table("fake_data").filter("total_spend >= 5000").write.mode("overwrite").saveAsTable("fake_data")#
...
display(spark.sql("DESCRIBE HISTORY fake_data"))

# COMMAND ----------

#Restore the table
spark.read.option("versionAsOf",4).table("fake_data").write.mode("overwrite").saveAsTable("fake_data")

# COMMAND ----------

#Verify that original count was restored
display(spark.read.table("fake_data").count())
...
# MAGIC %md
# MAGIC ## Exercise 2 - Solution SQL

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM fake_data WHERE email is NOT NULL

# COMMAND ----------

#Rename _sqldf so results do not get overwritten by any subsequent SQL Cells and to align to code below
silver_df = _sqldf

# COMMAND ----------

# MAGIC %md
# MAGIC ## Exercise 2 - Solution Python

# COMMAND ----------

silver_df = df.filter(df.email.isNotNull())
...
# MAGIC %md
# MAGIC ## Exercise 3 - SQL Solution

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE gold_spend AS
# MAGIC SELECT state,  sum(total_spend) as state_spend FROM silver_table
# MAGIC GROUP BY state

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM gold_spend

# COMMAND ----------

# MAGIC %md
# MAGIC ## Exercise 3 - Python Solution

# COMMAND ----------

# MAGIC %sql
# MAGIC --Reset table if ran SQL solution
# MAGIC DROP TABLE IF EXISTS gold_spend
...
df = spark.read.table("silver_table") \
    .groupBy("state") \
    .agg(sum("total_spend").alias("state_spend"))

# COMMAND ----------

df.write.mode("overwrite").saveAsTable("gold_spend")
